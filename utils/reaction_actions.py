...
def make_edit_tuple(atoms, edits):
    if type(atoms) == int:
        return tuple([atoms] + list(edits[:]))
    else:
        return tuple(atoms + list(edits[:]))


def judge(edits, ans, edits_atom, ans_atoms):
    if len(edits) != len(ans): return False
    ans_atoms = ans_atoms[0]
    lg1, lg2 = [], []
    for i in range(len(edits)):
        if ans[i][0] == 'Attaching LG':
            lg1.append(make_edit_tuple(ans_atoms[i], ans[i]))
            lg2.append(make_edit_tuple(edits_atom[i], edits[i]))
            continue
        if edits[i] != ans[i]:
            return False
    return set(lg2) == set(lg1)
...
class Termination(ReactionAction):
    ...
    def apply(self, mol: Mol) -> Mol:
        ...
        if all(int(bt) == 0 for bt in bond_stereo.values()) and all(
                int(chiral) == 0 for chiral in atom_chiral.values()):
