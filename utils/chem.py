...
def align_kekulize_pairs(r_mol: Mol, p_mol: Mol) -> Tuple[Mol, Mol]:
    ...
    # bond_info {(atom1,atom2):[btype,stero]}
    ...
    for bond in prod_new:
        if bond in react_new and (react_old[bond][0] == prod_old[bond][0]) and (
                react_new[bond][0] != prod_new[bond][0]):
...
def compare_mol(mola, molb):
    a_smi = canonicalize(Chem.MolToSmiles(mola))
    b_smi = canonicalize(Chem.MolToSmiles(molb))
    return a_smi == b_smi
...
def fix_Hs_Charge(mol: Mol) -> Mol:
    ...
    for atom in mol.GetAtoms():
        ...
        if charge == 0:
            ...
            # "-N=N+=N-"
            if atom_symbol == 'N' and len(
                    [b.GetBondTypeAsDouble() for b in atom.GetBonds()]) == 1 and bond_vals == 2 and atom.GetNeighbors()[
                0].GetSymbol() == 'N':
                atom.SetNumExplicitHs(0)
                atom.SetFormalCharge(-1)

            # "NC-"
            if atom_symbol == 'C' and len(
                    [b.GetBondTypeAsDouble() for b in atom.GetBonds()]) == 1 and bond_vals == 3 and atom.GetNeighbors()[
                0].GetSymbol() == 'N':
                atom.SetNumExplicitHs(0)
                atom.SetFormalCharge(-1)
