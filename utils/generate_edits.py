...
def generate_reaction_edits(rxn_smi: str, kekulize: bool = False, rxn_class: int = None,
                            rxn_id: str = None) -> ReactionData:
    ...
    for bond in react_bonds:
        ...
        if bond not in prod_bonds:
            ...
            if a1 in p_amap_idx and a2 in p_amap_idx:
                ...
                bond_edits_atom.add(a1)
                bond_edits_atom.add(a2)
    # atom_info{atom_map:[atom.GetNumExplicitHs(), int(atom.GetChiralTag())]}
    ...
    for atom in react_atoms:
        if atom not in prod_atoms:
            atoms_only_in_react.append(atom)  # atom_map
    # find changed atoms
    if len(edits_atom) == 0:
        # 完全没有边的变化，必然有原子上的变化
    ...
    for bond in react_mol.GetBonds():
        ...
        if a1 not in atoms_only_in_react and a2 in atoms_only_in_react:
            ...
            for smi in frags1_smi:
                ...
                if all(a.GetAtomMapNum() in atoms_only_in_react for a in mol.GetAtoms()):  # 完全在mol中
