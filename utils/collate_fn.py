...
def prepare_edit_labels(graph_batch: List[MolGraph], edits: List[Any], edit_atoms: List[Any], bond_vocab: List,
    ...
    return edit_labels, lg_edits


def build_edit_tensors(cur_graph_tensors, edits_info, is_initial, vocab, prod_graphs):
    edits, edit_atoms = edits_info
    # edit_atoms is the atom_map number not true index
    graph_tensors, scopes = cur_graph_tensors
    atom_scopes, bond_scopes = scopes
    f_atoms, f_bonds, a2b, b2a, b2revb, undirected_b2a = graph_tensors
    tot_vocab = vocab
    if is_initial:
        ini = tot_vocab.get("Initialize")
        res = torch.ones((f_atoms.shape[0], 1)) * ini
        edit_mask = torch.ones((f_atoms.shape[0], 1)) * ini
        # true_edit = torch.ones((f_atoms.shape[0], 1))
    else:
        null_op = tot_vocab.get("Null")
        res = torch.ones((f_atoms.shape[0], 1)) * null_op
        edit_mask = torch.ones((f_atoms.shape[0], 1)) * null_op
        true_edit = torch.ones((f_atoms.shape[0], 1)) * null_op
        bsz = len(edits)
        for i in range(bsz):
            prod_graph = prod_graphs[i]
            e, e_atoms = edits[i], edit_atoms[i]
            # transform to the atom_index in graph
            if type(e_atoms) is not list:
                e_atoms = [e_atoms]
            for atom in e_atoms:
                atom = prod_graph.amap_to_idx[atom]
                assert atom_scopes[i][0] + atom < res.shape[0]
                res[atom_scopes[i][0] + atom] = tot_vocab.get(e, 0)
            edit_mask[atom_scopes[i][0]:atom_scopes[i][0] + atom_scopes[i][1]] = tot_vocab.get(e, 0)
    return res.long(), edit_mask.long()

def get_batch_graphs(graph_batch: List[MolGraph], use_rxn_class: bool = False) -> Tuple[
    torch.Tensor, List[Tuple[int]]]:
    """
...
def get_batch_graphs_lgs(graph_batch: List[MolGraph], lg_mol, lg_edits, use_rxn_class: bool = False, add_cls=False) -> \
        Tuple[
            torch.Tensor, List[Tuple[int]]]:
    """
    Featurization of a batch of molecules.
    ...
    graph_tensors, scopes = get_batch_graphs(graph_batch, use_rxn_class=use_rxn_class)
    lg_tensors, lg_scopes = None, None
    if any(lg_mol):
        # print('attach LG')
        lg_idx = [i for i in range(len(lg_edits)) if lg_edits[i] is not None]
        lg_tensors, lg_scopes = get_batch_graphs(lg_mol, use_rxn_class=False)
        lg_scopes = tuple(list(lg_scopes[:]) + [lg_idx])

    return (graph_tensors, scopes), (lg_tensors, lg_scopes)
