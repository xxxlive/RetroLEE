...
def main():
    ...
    checkpoint = torch.load(os.path.join(exp_dir, 'epoch_{}.pt'.format(args.epoch)))
    ...
    df_file = os.path.join(exp_dir, 'result_table.csv')
    file_num = 1
    ...
    with open(pred_file, 'a') as fp:
        for idx in p_bar:
            ...
            pred_res.append({'product': p, 'reactant': r_smi, 'predictions': '\t'.join(tmp_predictions)})
