...
from models.model_utils import get_tmp_edit_tensor
...
class DamnSearch:
    ...
    def process_path(self, path, rxn_class):
        ...
        # build edit data
        edit = None if not path['edits'] else path['edits'][-1]
        edit_atom = None if not path['edits_atom'] else path['edits_atom'][-1]
        edit_data = get_tmp_edit_tensor(prod_tensors=prod_tensors, edit=edit, edit_atom=edit_atom,
                                        prod_graph=path['prod_graph'],
                                        tot_vocab=self.model.tot_vocab)
        edit_logits, state, state_scope = self.model.compute_edit_scores(
            prod_tensors, path['scopes'], path['state'], path['state_scope'], edit_data=edit_data)
