...
def prepare_data(args: Any) -> None:
    ...
    cnt = 0
    for idx, rxn_data in enumerate(tqdm(rxns_data)):
        graph_seq = []
        ...
        for i, edit in enumerate(rxn_data.edits):
            ...
            if edit == 'Terminate':
                graph = RxnGraph(prod_mol=Chem.Mol(
                    int_mol), edit_to_apply=edit, reac_mol=Chem.Mol(r_mol), rxn_class=rxn_data.rxn_class,
        ...
        if len(graph_seq) == 0 or final_smi is None:
            ...
            sys.stdout.flush()
            continue
        joblib.dump(graph_seq,os.path.join(savedir, f'tot_batch_data_{cnt}.pt'))
        cnt += 1
    ...
    sys.stdout.flush()
    print("Saving..")
    # torch.save(batch_graphs, os.path.join(savedir, f'tot_batch_data.pt'))
    print(lg_cnt)
    joblib.dump(lg_cnt, 'lg_cnt.txt')
    # for i in tqdm(range(len(batch_graphs))):
    #     joblib.dump(batch_graphs[i], os.path.join(savedir, f'tot_batch_data_{i}.pt'))
    print('Saved..')
